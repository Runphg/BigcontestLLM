...
embedding_model = AutoModel.from_pretrained(model_name).to("cuda" if torch.cuda.is_available() else "cpu")
chat_session = model.start_chat(history=[])  # 채팅 세션 객체 반환

print(f'Device is {device}.')


# FAISS 인덱스 로드 함수
# 인덱스에는 임베딩 벡터들이 저장돼있음
...
def generate_response_with_faiss(question, df, embeddings, embed_text, time, local_choice, chat_session, k=3):
    filtered_df = df
...
if st.session_state.messages[-1]["role"] != "assistant":
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            response = generate_response_with_faiss(prompt, df, embeddings, embed_text, time, local_choice)
